...
#include "../../../include/fnx/utils/coro.hpp"

namespace FNX::Utils::Coro {

template <class T>
__FNX__CORO_NS::suspend_always
generator<T>::promise_type::initial_suspend() {
  return __FNX__CORO_NS::suspend_always{};
}
...
template <class T>
__FNX__CORO_NS::suspend_never
generator<T>::promise_type::return_void() {
  return __FNX__CORO_NS::suspend_never{};
}
...
async<T>::awaitable_type::await_suspend(
    __FNX__CORO_NS::coroutine_handle<> awaiting) {
  this->coro.resume();
  return awaiting;
}
