...
// is no need to worry about binary size. Lesser `-O` reduces the
// time required to pre-compile the code, while greater `-O`
// increases the compiled code performance.
class JIT : Shared::BC {
public:
